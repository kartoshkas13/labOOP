    ...
    public class ComplexNumber : StringBase, IComplexActions
    {
        ...
        private void Parse()
        {
            try
            {
                ...
                if (!s.EndsWith("i"))
                    throw new Exception("Рядок має закінчуватись на i");
                ...
                if (!IsValidPart(realPart) || !IsValidPart(imagPart))
                    throw new Exception("Частини числа містять неправильні символи");
        ...
        {
            if (other == null)
                throw new Exception("Об'єкт для порівняння відсутній");
